...
- [x] Fun/entertaining teacher-only stats — submission time-of-day + day-of-week (computed live from data/submissions-log.json's receivedAt, new "Submission patterns" section on teacher-student.html), a computed "Vibe check" zodiac-style type (rule-based off mastery+motivation, no new data field, also shown as a column on teacher-overview.html), a manually-tracked Responsiveness score/note, and free-text Personality tags. Responsiveness + Personality wired into the existing "Add metric entry" form (course.metrics.responsiveness / .personality) and automation/zenith-data-writer.gs's METRIC_ARRAY_TYPES_/AP_SCORE_FIELDS_ — same "not deployed yet, reference copy only" caveat as the rest of zenith-data-writer.gs.

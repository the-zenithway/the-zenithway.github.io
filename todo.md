...
## Done
- [x] Teachers can schedule a notification email to one of their classes for a specific future time — new "Schedule a notification" form on teacher.html (class/subject/message/send-time), backed by scheduleNotification/cancelScheduledNotification in zenith-data-writer.gs writing to new data/scheduled-notifications.json. Actual sending is a separate system: a new GitHub Actions cron job (every 15 min, automation/notifications/send-scheduled-notifications.js) polls that file for due Pending rows and emails everyone in the target class via the existing Gmail/nodemailer setup. Doesn't cover "set IRL sessions" (calendar creation) — just the scheduled-email half of that todo line above.
